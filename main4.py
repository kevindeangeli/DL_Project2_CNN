...
class ConvolutionalLayer():
    ...
    def deltaSum(self):
        x=4
        #The problem with my code is that max polling is already sending the matrix of deltas. So this function will have to accept the matrix
        #and compute the convolution with the weights.
        #rememerber to consider the number of kernels and the dimension of the delta matrix
        #compute the
        #in reality, this function is never used because the conv layer is always the first layer.

    def backpropagation(self, deltaMatrix):
        outputs = []
        for index, kernel in enumerate(self.kernels):
            outputs.append(self.convolve(kernel = kernel, input=deltaMatrix))
        print(outputs)
        print(deltaMatrix)



    # rotate matrix of deltas,
    # compute the updates for each kernel
    # Compute the new
    ...
    #input should be a list of matrices:
        ...
        for matrix in matrix_list:
            ...
            for i in range(self.numberOfNeurons):
                ...
                max_index= np.reshape(np.array(np.where(matrix[i1:i2, j1:j2] == max_val)), (1, 2))[0]
                max_index[0] += i1
                max_index[1] += j1
    ...
    def backPropagate(self, target):
        ...
        for i in range(2,layersCounter):
            ...
            if type(self.layers[-i + 1]) == FlattenLayer or type(self.layers[-i + 1]) == MaxPoolingLayer:
                self.layers[-i + 1].deltaSum(deltaArr)
                #continue
            else:
